...
def validate(post_details):
    if post_details.tab == "trakt":
        result = validate_trakt(post_details.trakt_client_id, post_details.trakt_client_secret, post_details.trakt_pin)
    else:
        result = validate_mal(post_details.code_verifier, post_details.mal_client_id, post_details.mal_client_secret, post_details.mal_local_url)
...
def validate_trakt(client_id, client_secret, pin):
    ...
    if response.status_code != 200:
        response = "Trakt Error: Invalid trakt pin. If you're sure you typed it in correctly your client_id or client_secret may be invalid"
    else:
        headers = {
    ...
    if validation_response.status_code == 423:
        response = "Trakt Error: Account is locked; please contact Trakt Support"
    else:
        response = f"########## GENERATED BY TRAKTAUTH ##########\ntrakt:\n  client_id: {client_id}\n  client_secret: {client_secret}\n  authorization:\n    access_token: {response.json()['access_token']}\n    token_type: {response.json()['token_type']}\n    expires_in: {response.json()['expires_in']}\n    refresh_token: {response.json()['refresh_token']}\n    scope: {response.json()['scope']}\n    created_at: {response.json()['created_at']}\n  pin:\n############################################\n"
...
def validate_mal(code_verifier, client_id, client_secret, localhost_url):
    ...
    if not match:
        response = "Couldn't find the required code in that URL."
    else:
        code = match.group(1)
